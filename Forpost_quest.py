...
def writing(print_text, us_id, k_b):
    global b  # Переменная для отлова ошибки нумерации сообщений внутри вк сервера
                        ...
                        if player.us_id == user_id:
                            ...
                            if player.part == 1:
                                if player.room == 'exit':
                                    ...
                                    print(str(user_id) + " начал заново")
                                ...
                                if player.room == 'reading numbers':
                                    ...
                                    if text == 'Прошёл день':
                                        writing_only_text('Конец рабочей смены.\nМастера и архитекторы отчитываются в '
                                                          ...
                                                          'отправляются на заслуженный отдых. Только солдаты остаются '
                                                          'на постах, готовые в случае опасности поднять тревогу.',
                                                          user_id)
                                        ...
                                        if player.food <= 0:
                                            writing('Население замка столкнулось с голодом. Продолжение строительства '
                                                    'невозможно в связи с несоблюдением прав рабочих. Вас вышвыривают '
                                                    'на улицу без выплаты золота.', user_id, keyboards.keyboard_16)
                                            player.part = 1
                                            player.room = 'exit'
                                        else:
                                            player.room = 5
                                ...
                                if player.move == 1:
                                    ...
                                    if int(player.enemy_num) <= 0 or (player.turn == 10 and int(player.soldiers) > 0):
                                        if player.turn == 10:
                                            writing('Победа! Враги не смогли прорваться в крепость и решили отступить.',
